...
class DefenseServer(Server):
    def __init__(self, cf=None, model=None, attack_type=None, attacker_ratio=None, dataset = None, experiment_name = 'debug'):
        ...
        # Saving directory
        self.dir_path = f"Results/experiment_name/{self.dataset}/FedCAM_conv/{self.cf['data_dist']}_{int(attacker_ratio * 100)}_{attack_type}"
        ...
        self.cvae = ConvCVAE(input_dim=cf["cvae_input_dim"],
            representation_dim = 2807, 
    ...
    def compute_client_errors(self, clients, gm): 
        return self.compute_reconstruction_error(clients, gm)
    
    def filter_clients(self, selected_clients): 
        # Error computations and handling step
        clients_re_np = np.array(self.compute_client_errors(selected_clients, self.gm))
    ...
    def train_cvae(self):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, activation in enumerate(loop):
                ...
                target = self.cvae.get_repr(activation)
                loss = Utils.cvae_loss(recon_batch, target, mu, logvar)
    ...
    def compute_reconstruction_error(self, selected_clients, benign_gm = None):
        self.cvae.eval()
        ...
        for client_act in clients_act:
            ...
            target = self.cvae.get_repr(client_act)
            mse = F.mse_loss(recon_batch, target, reduction='mean').item()
